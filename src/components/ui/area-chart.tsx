                ...
                <>
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                    ...
                    {item.value !== undefined && (
                      <span className="text-foreground font-mono font-medium tabular-nums">{(item.value as number).toLocaleString()}</span>
                    )}
                  </div>
                </>
              )}
...
/* ─── Hatched pattern helper — use inside <defs> ─── */
export function HatchedPattern({ id, color, animated = true }: { id: string; color: string; animated?: boolean }) {
  return (
    <pattern id={id} x="0" y="0" width="6.81" height="6.81"
      patternUnits="userSpaceOnUse" patternTransform="rotate(-45)" overflow="visible">
