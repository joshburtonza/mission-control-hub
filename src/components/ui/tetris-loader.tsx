...
export default function TetrisLoading({
        ...
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border border-white/10 transition-all duration-100 ${
              cell.filled ? `${cell.color} scale-100` : 'scale-95'
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className={`border border-white/20 ${config.padding}`}>
        {renderGrid()}
      </div>
      {showLoadingText && (
        <p className="text-[11px] font-medium tracking-widest uppercase" style={{ color: 'var(--tc-35)' }}>
          {loadingText}
        </p>
      )}
    </div>
